...
var statusLabel, drawingCanvas;

const teamMembers = [
    {name: "Juraj", photo: "https://ca.slack-edge.com/T02M3Q53Z-U05K066D5K6-4d3063c686f4-512", available: true},
    {name: "Veronika", photo: "https://ca.slack-edge.com/T02M3Q53Z-UL0U3UFB2-b896d30bd91c-512", available: true},
    {name: "Filip", photo: "https://ca.slack-edge.com/T02M3Q53Z-UDN9QFV7C-95f5307dd2fc-512", available: true}, 
    {name: "Braňo", photo: "https://ca.slack-edge.com/T02M3Q53Z-U4LKQ2ALT-87658e8adf5b-512", available: true},
    {name: "Honza", photo: "https://ca.slack-edge.com/T02M3Q53Z-U04DA9185DM-e4c2a5e47f48-512", available: true},
    {name: "Laci", photo: "https://ca.slack-edge.com/T02M3Q53Z-U6DB8TM4M-e449e29f676d-512", available: true},
];
...
function preloadImages(callback) {
    ...
    teamMembers.forEach(teamMember => {
        teamMember.image = new Image();
        teamMember.image.addEventListener('load', () => {
            loaded++;
            if (loaded == teamMembers.length) callback();
        }, true);
        teamMember.image.src=teamMember.photo;
    });
}
...
function checkEndDrag(e) {
    if (mouseConstraint) {
        ...
        if (wheelSpinning === false && wheelStopped === true) {
            if ( Math.abs(wheel.body.angularVelocity) > 2.5) {
                wheelSpinning = true;
                wheelStopped = false;
            }
        }
...
function initPhysics() {
    ...
    wheel = new Wheel(wheelX, wheelY, wheelRadius, 36, teamMembers, 0.25, 11.5);
...
function update() {
    ...
    if (wheelSpinning === true && wheelStopped === false &&
        ...
        setTimeout(
            () => {
                teamMembers.splice(getTeamMemberIndex(wheel.body.angle), 1);
                wheel.predrawWheel();
            },
            2000
        );
...
function draw() {
    ...
    statusLabel.innerHTML = teamMembers[getTeamMemberIndex(wheel.body.angle)].name
}

function getTeamMemberIndex(angle) {
    teamMemberAngle = TWO_PI / teamMembers.length;
    teamMemberIndex = Math.round(wheel.body.angle / teamMemberAngle) % teamMembers.length;
    if (teamMemberIndex < 0) {
        teamMemberIndex += teamMembers.length;
    }
...
// wheel of fortune
/////////////////////////////
function Wheel(x, y, radius, pins, teamMembers, pinRadius, pinDistance, onReady) {
    this.x = x;
    this.y = y;
    this.radius = radius;
    this.pins = pins;
...
Wheel.prototype = {
    ...
    draw:function() {
        ...
        if (this.predrawn == null) this.predrawWheel();
        ctx.rotate(-this.body.angle);
        ctx.drawImage(this.predrawn, - this.pRadius, - this.pRadius);
        ctx.restore();
    },

    predrawWheel: function () {
        var tempCanvas = document.createElement("canvas"),
        ctx = tempCanvas.getContext("2d");
        tempCanvas.width = this.pRadius * 2;
        tempCanvas.height = this.pRadius * 2;
        ctx.translate(this.pRadius, this.pRadius);

        var deltaPI = TWO_PI / this.pins;
        for (var i = 0; i < this.pins; i++) {
            ctx.fillStyle = (i % 2 === 0) ? '#BD4932' : '#FFFAD5';
            ctx.beginPath();
            ctx.arc(0, 0, this.pRadius, i * deltaPI, (i + 1) * deltaPI);
            ctx.lineTo(0, 0);
            ctx.closePath();
            ctx.fill();
        }
        ...
        ctx.save();
        this.predrawTeamMembers(ctx);
        this.predrawn = tempCanvas;
    },

    predrawTeamMembers: function (ctx) {
        deltaPI = TWO_PI / this.teamMembers.length;
        headRadius = this.pRadius / 3.5;
        for (var i = 0; i < this.teamMembers.length; i++) {
            photo = this.teamMembers[i].image;
            ctx.beginPath();
            ctx.arc(0, - this.pRadius * 2 / 3, headRadius, 0, Math.PI * 2,true);
            ctx.closePath();
            ctx.clip();
            if (!this.teamMembers[i].available) {
                console.log("here");
